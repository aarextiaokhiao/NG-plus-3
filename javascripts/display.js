...
function replicantiDisplay() {
	...
	if (unl) {
		...
		el("replicantiapprox").innerHTML = mod.ngp3 && player.dilation.upgrades.includes("ngpp1") && hasTimeStudy(192) && player.replicanti.amount.gte(Number.MAX_VALUE) ? 
			...
			(tmp.rep.warp ? "<br><b>Replicanti Warp: " + timeDisplayShort(tmp.rep.interval.div(tmp.rep.dupRate), true, 2) + " interval → ^" + shorten(tmp.rep.warp) + " slowdown!</b><br>(10x faster until " + shortenCosts(tmp.rep.warp_lim) + ")" : "") :
...
function initialTimeStudyDisplay(){
	...
	el("121desc").textContent = "Currently: " + shortenMoney(tsMults[121]()) + "x"
	el("123desc").textContent = "Currently: " + Math.sqrt(1.39*player.thisEternity/10).toFixed(1) + "x"
	el("141desc").textContent = "Currently: " + shortenMoney(tsMults[141]()) + "x"

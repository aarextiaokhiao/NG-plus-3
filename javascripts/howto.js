let HOW_TO = [
	{
		...
		<br>
		Each completed challenge will award an autobuyer. You can run them multiple times (though only the first time grants a reward), and they can be exited at any time via the "Exit Challenge" button on the challenge tab.
		<br>
		...
		desc() {
			...
			if (feature == 3) msg += `<b>Phantomal Paradigms</b><br>
				Phantomal Paradigms are a Big Rip replacement to Dimensional Shifts that cause scaling changes. Any Phantomal Paradigm removes the cost scaling of a Antimatter Dimension tier (except the 8th), but however has a fixed per-ten bonus. These only reset on Fundament.`
		...
		desc: `
			...
			After going Fundament, you can click the same button again to emit stored time, which skips time onto a chosen feature.
		`
	}, {
		mod: _ => ngp3,

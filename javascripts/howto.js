let HOW_TO = [
	{
		...
		layer: "eternity",
		desc: `Meta Dimensions are a new type of Dimensions that resemble Antimatter Dimensions. They can be unlocked by buying 1e24 TT dilation study. Meta-dimension 1 produces meta-antimatter, which makes dimension boosts more powerful. A meta-dimension 2 produces meta-dimension 1 and so on until the 8th.<br><br>
		...
		Positrons provide a boost to your multiplier per 10 dimensions bought, but in order to use them, you have to sacrifice your regular galaxies.<br><br>
		
		The number of Positrons you get is equal to the number of galaxies you sacrificed times the number of Positrons per galaxy you currently have. This number starts at 2 Positrons per galaxy, and can be upgraded using either time theorems, dilated time, meta-antimatter, or meta-dimension boosts. Each upgrade increases the Positrons per galaxy by 0.25.`
	}, {
		mod: _ => ngp3,
		...
		tab: "ant",
		layer: "quantum",
		desc: `You can make Duplicants by resetting your Replicanti amount after reaching certain requirements (starts at e3,000,000, increases e100,000 each time).<br>
		...
		Pilon anti-energy slows down pilon energy production until a cap of anti-energy is reached, where pilon energy production completely stops. When this happens, the only way to get more pilon energy is to get more pilon charge or go quantum.<br><br>
		...
		desc() {
			...
			You can undo this to gain Space Shards based on how much antimatter you gained. You can spend Space Shards to buy various progression upgrades, either by giving a boost or a quality-of-life feature that only applies in Big Rips, or letting you keep something inside Big Rips.<br><br>`
			...
			if (feature >= 1) msg += `<b>Break Eternity</b><br>
				 If you broke Eternity and are currently in Big Rip, Galaxies and Time Dimensions will work, but scale faster. On Eternity, you gain eternal matter based on your time shards. You can spend Eternal Matter to buy 7 new big-rip-exclusive upgrades (one of which is repeatable).<br><br>`
			...
			if (feature == 3) msg += `<b>Phantomal Paradigms</b><br>
				Phantomal Paradigms are a Big Rip replacement to Dimensional Shifts that cause scaling changes. Each Phantomal Paradigm removes the cost scaling of a Antimatter Dimension tier (except the 8th), but that dimension will have a constant per-ten bonus. These only reset on Fundament.`
		...
		layer: "fundament",
		desc: `
			This is the fourth major layer of NG+3 that unlocks on reaching the goal in Big Rip runs. You can perform a Fundament only if you're in Big Rip. This resets all prior progress as well as all Quantum features and Speedrun Milestones, and gives you Spectral Particles based on your current antimatter during this Big Rip. Like the Quantum layer, you'll also keep some quality-of-life features beforehand.
			<br><br>

			<b>Brave Milestones</b><br>
			Brave Milestones can be achieved by Fundamenting while under a specific amount of Quantums. Likewise, each Brave Milestone provides you with a reward that greatly boosts progress and makes Fundaments much faster and more efficient.
			<br><br>

			<b>Automators</b><br>
			Automator Charge is based on net Quarks and other stuff you'll gain later. You unlock an Automator on passing a certain threshold. Automator Power is your best-ever Automator Charge, but is used on enabling an Automator. Automators automate various features such as Nanofield and Big Rips, as long the your used power doesn't exceed the cap.
		`
	}, {
		mod: _ => ngp3,
		req: _ => ghSave?.times,
		...
		desc: `
			...
			Neutrinos are used to boost things dependent on all kinds, and can be spent for upgrades or a repeatable which gives 3x Spectral Particles per purchase.<br>
			More Neutrino upgrades can be unlocked between 3rd Fundament and 10th Fundament resets.
		`
	}, {
		mod: _ => ngp3,
		...
		layer: "fundament",
		desc: `
			Tachyonic Galaxies generate Photons, which strengthen Lights. To unlock more colors of Lights, you must gain more Photons. However, going Fundament resets Photons.
			<br><br>
		...
		desc: `
			...
			A W Charger provide strength onto chargers adjacent to it. W+ Chargers add strength, but W- Chargers reduce strength.<br>
			...
			Some milestones can boost Bosons, so make sure to setup presets! I recommend joining the NG+3 Discord server for further help. At least couple presets are recommended for progression.
		`
	}, {
		mod: _ => ngp3,
		req: _ => ghSave?.hb?.unl,
		...
		layer: "fundament",
		desc: `
			At ??? Bosons, you can reset W & Z Bosons for Higgs. The first Higgs reset unlocks Higgs Field. Further Higgs resets require more Higgs. Unlike Dimensional Boosts and Galaxies, Higgs can be increased by any amount in one run.
			<br><br>

			Higgs Field has a grid of swappable boosts. You passively produce Higgs Mass which strengthens all boosts from Higgs Field. Later rows provide weak effects at first, but gradually increase to be effective as earlier rows by gaining more Higgs Mass.
		`,
	}/*, {
		mod: _ => ngp3,
		req: _ => ghSave?.gw?.unl,

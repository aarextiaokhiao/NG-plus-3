let HOW_TO = [
	{
		...
		title: "Black Hole",
		layer: "eternity",
		desc: `On spending e4K EP, several resources can be exchanged for Remnants. Your Black Hole generates hunger. Feeding with Remnants not only reduces hunger, but also unlocks various boosts based on the power you would attain.`
	}, {
		...
		desc() {
			...
			if (feature >= 1) msg += `<b>Break Eternity</b><br>
				After breaking Eternity: Galaxies and Time Dimensions will work in Big Rips, but scale much faster. On Eternity, you gain eternal matter based on your time shards. You can spend Eternal Matter to buy 7 new big-rip-exclusive upgrades (one of which is repeatable).<br><br>`
			...
			else if (feature == 2) msg += `<b>Get Photons to unlock a new subfeature.</b>`
		...
		layer: "fundament",
		desc: `
			This is the 4th major layer of NG+3 that unlocks on reaching the goal during this Big Rip. You can perform a Fundament only if you're in Big Rip. This resets all prior progress as well as all Quantum features and Speedrun Milestones, and gives you Spectral Particles based on your current antimatter during this Big Rip. You'll also keep some QoL features beforehand.
			<br><br>
		...
		layer: "fundament",
		desc: `
			There are 3 kinds of Neutrinos: Electron, Mu, and Tau. You gain Neutrinos of a single kind per 1 Antimatter Galaxy gained.<br>
		...
		layer: "fundament",
		desc: `
			Tachyonic Galaxies generate Photons, which strengthen Lights. To unlock more colors of Lights, you must gain more Photons. Each color requires more Photons than the prior. However, going Fundament resets Photons.
			<br><br>
			...
			After going Fundament, you can click the same button again to emit stored time, which causes a chosen feature to time skip.<br>
		...
		layer: "fundament",
		desc: `
			At ??? Bosons, you can reset W & Z Bosons for Higgs. The first Higgs reset unlocks Higgs Field. Further Higgs resets require more Higgs. Unlike Dimensional Boosts and Galaxies, Higgs can be increased multiplely in one run.<br><br>

			Higgs Field has a grid of swappable boosts. You also produce Higgs Mass which strengthens Higgs Field. Later rows start off weak and gradually increase to be effective as earlier rows by strengthening Higgs Field.
		`,
	}/*, {
		mod: _ => ngp3,
		req: _ => ghSave?.gw?.unl,

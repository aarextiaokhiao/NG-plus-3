...
function updateNewPlayer(reseted) {
    ...
    if (modesChosen.ngpp > 1) {
        player.aarexModifications.newGame3PlusVersion = 1.25
        player.dbPower = 1
        player.peakSpent = 0
...
function getRemoteGalaxyScalingStart() {
	...
	if (player.masterystudies) {
		...
		if (player.masterystudies.includes("t252")) n += Math.floor(player.dilation.freeGalaxies/9)
		if (player.masterystudies.includes("t253")) n += Math.floor(extraReplGalaxies*4/3)
	}
	return n
}
...
function getGalaxyRequirement() {
    ...
    if (player.galaxies >= remoteGalaxyScalingStart) {
        amount = Math.floor(amount * Math.pow(1.002, (player.galaxies-remoteGalaxyScalingStart-1)))
    }
...
function getDilPower() {
	...
	if (player.masterystudies) if (player.masterystudies.includes("t264")) ret *= Math.pow(player.galaxies+1,0.25)*2
	return ret
}
...
function updateDimensions() {
    if (document.getElementById("antimatterdimensions").style.display == "block" && document.getElementById("dimensions").style.display == "block") {
        ...
        document.getElementById("secondResetLabel").textContent = (player.galaxies < 1400 ? (player.galaxies < getGalaxyCostScalingStart() ? '' : player.galaxies < getRemoteGalaxyScalingStart() ? 'Distant ' : 'Remote ') + 'Antimatter' : player.galaxies < getRemoteGalaxyScalingStart() ? 'Non-remote DM' : 'Dark Matter') + ' Galaxies ('+ getFullExpansion(player.galaxies) + ((extraGals + player.dilation.freeGalaxies) > 0 ? ' + ' + getFullExpansion(extraGals)  + (player.dilation.freeGalaxies > 0 ? ' + ' + getFullExpansion(player.dilation.freeGalaxies) : '') : '') +'): requires ' + getFullExpansion(getGalaxyRequirement()) + ' '+DISPLAY_NAMES[player.currentChallenge === 'challenge4' ? 6 : 8]+' Dimensions';
    }
    ...
    if (document.getElementById("eternityupgrades").style.display == "block" && document.getElementById("eternitystore").style.display == "block") {
        ...
        document.getElementById("eter2").innerHTML = "Infinity Dimension multiplier based on eternities ("+(player.eternities<1e12?"(x/200)^log4(2x)":"x^log10(x)^3.75")+")<br>Currently: "+shortenMoney(getEU2Mult())+"x<br>Cost: 10 EP"
...
function updateInfCosts() {
    ...
    if (document.getElementById("replicantis").style.display == "block" && document.getElementById("infinity").style.display == "block") {
        document.getElementById("replicantichance").innerHTML = "Replicate chance: "+getFullExpansion(Math.round(player.replicanti.chance*100))+"%" + (isChanceAffordable() ? "<br>+1% Cost: "+shortenCosts(player.replicanti.chanceCost)+" IP" : "")
        let replGalOver = 0
...
function sacrifice(auto = false) {
    ...
    var boost = calcSacrificeBoost()
    if (player.galacticSacrifice) {
      if (player.galacticSacrifice.upgrades.includes(11)) {
        player.secondPow = player.secondPow.times(boost)
        player.fourthPow = player.fourthPow.times(boost)
        player.sixthPow = player.sixthPow.times(boost)
      }
    }
    player.eightPow = player.eightPow.times(boost)
...
document.getElementById("bigcrunch").onclick = function () {
    ...
    if ((player.money.gte(Number.MAX_VALUE) && !player.currentChallenge.includes("post")) || (player.currentChallenge !== "" && player.money.gte(player.challengeTarget))) {
        ...
        if (player.currentEternityChall !== "eterc14" && player.money.gte("1e5000") && isIC3Trapped()) player.challenges.push("postc3")
...
function buyDilationUpgrade(id, costInc) {
    if (id > 3 && id != 11) { // Not rebuyable
        ...
        if (id == 15 && player.masterystudies) document.getElementById("masterystudyunlock").style.display=""
    } else { // Is rebuyable
...
function getPassiveTTGen() {
	...
	if (log > 45) return Decimal.pow(10, 30 + Math.sqrt(log * 15 - 450)).div(20000)
...
var scaleStarts = [null, 1e9, 1e9, 59, 18]
...
function updateDilation() {
    if (document.getElementById("dilation").style.display == "block" && document.getElementById("eternitystore").style.display == "block") {
        ...
        document.getElementById("dilatedGalaxies").textContent = getFullExpansion(player.dilation.freeGalaxies)
    }
}
...
setInterval(function() {
    ...
    document.getElementById("eterc13goal").textContent = "Goal: "+shortenCosts(new Decimal("1e15000000").times(new Decimal("1e600000").pow(ECTimesCompleted("eterc13"))).max(new Decimal("1e15000000"))) + " IP"
    document.getElementById("eterc13completed").textContent = "Completed "+ECTimesCompleted("eterc13")+" times."

    document.getElementById("eterc14goal").textContent = "Goal: "+shortenCosts(new Decimal("1e835000").times(new Decimal("1e16000").pow(ECTimesCompleted("eterc14"))).max(new Decimal("1e835000"))) + " IP"
...
function gameLoop(diff) {
    ...
    if (player.money.gte(Number.MAX_VALUE) && (!player.break || (player.currentChallenge !== "" && player.money.gte(player.challengeTarget)) || (player.currentEternityChall !== "eterc14" && isIC3Trapped() && player.money.gte("1e5000")))) {
    ...
    if ((player.galacticSacrifice ? (player.galacticSacrifice.times > 0 || player.infinitied > 0 || player.eternities != 0) : false) && !isEmptiness) {
    ...
    if ((player.infinitied > 0 || player.infinityPoints.gt(0) || player.eternities !== 0) && !isEmptiness) {
    ...
    document.getElementById("ec13reward").textContent = "Reward: Increase the power of meta-antimatter. ("+(10+ECTimesCompleted("eterc13")*0.2)+"x)"
    document.getElementById("ec14reward").textContent = "Reward: Time Dimensions boosts IC3 reward "+(ECTimesCompleted("eterc14")*2)+" times."

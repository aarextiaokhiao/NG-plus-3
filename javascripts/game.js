...
function updateNewPlayer(reseted) {
    if (reseted) {
        var modesChosen = {
            ...
            ngpp: player.aarexModifications.newGamePlusPlusVersion === undefined ? false : player.aarexModifications.newGame3PlusVersion === undefined ? true : 2,
...
function getGalaxyRequirement() {
    ...
    if (player.aarexModifications.newGameMinusMinusVersion) amount -= 20;
...
function updateDimensions() {
    if (document.getElementById("antimatterdimensions").style.display == "block" && document.getElementById("dimensions").style.display == "block") {
        ...
        document.getElementById("resetLabel").textContent = 'Dimension ' + (isShift ? "Shift" : player.resets < 3e5 ? "Boost" : "Supersonic") + ' ('+ getFullExpansion(player.resets) +'): requires ' + getFullExpansion(shiftRequirement.amount) + " " + DISPLAY_NAMES[shiftRequirement.tier] + " Dimensions"
        ...
        document.getElementById("secondResetLabel").textContent = (player.galaxies < getGalaxyCostScalingStart() ? '' : player.galaxies < 800 ? 'Distant ' : 'Remote ') + 'Antimatter Galaxies ('+ getFullExpansion(player.galaxies) + ((extraGals + player.dilation.freeGalaxies) > 0 ? ' + ' + getFullExpansion(extraGals)  + (player.dilation.freeGalaxies > 0 ? ' + ' + getFullExpansion(player.dilation.freeGalaxies) : '') : '') +'): requires ' + getFullExpansion(getGalaxyRequirement()) + ' '+DISPLAY_NAMES[player.currentChallenge === 'challenge4' ? 6 : 8]+' Dimensions';
    }
    ...
    if (document.getElementById("stats").style.display == "block" && document.getElementById("statistics").style.display == "block") {
        ...
        if (player.eternities == 0) {
            ...
            document.getElementById("eternitied").textContent = "You have Eternitied " + getFullExpansion(player.eternities.toString()) + " times."
    ...
    if (document.getElementById("infinity").style.display == "block") {
        if (document.getElementById("preinf").style.display == "block") {
            ...
            else if (ECTimesCompleted("eterc6") == 5 || player.dimensionMultDecrease == 3) document.getElementById("postinfi42").innerHTML = "Dimension cost multiplier increase <br>"+player.dimensionMultDecrease.toFixed(0)+"x"
            else document.getElementById("postinfi42").innerHTML = "Dimension cost multiplier increase <br>"+player.dimensionMultDecrease.toFixed(1)+"x"
    ...
    if (document.getElementById("dilation").style.display == "block") {
        if (player.dilation.active) {
            let gain = getDilGain()
            let req = Decimal.pow(10, Math.pow(player.dilation.totalTachyonParticles / (Math.pow(3, player.dilation.rebuyables[3])), 1 / getDilExp()) * 400);
...
function updateCosts() {
    ...
    for (var i=1; i<9; i++) {
        ...
        if (player.meta) {
            document.getElementById("meta"+i).innerHTML = "Cost: " + shortenDimensions(player.meta[i].cost) + " MA"
            document.getElementById("metaMax"+i).innerHTML = "Until 10: " + shortenDimensions(getMetaMaxCost(i)) + " MA"
        }
    }
}
...
function updateChallenges() {
    try {
        ...
        for (var i=1; i<9; i++) document.getElementById("postc"+i+"div").style.display = (player.postChallUnlocked >= i || (i == 3 && player.aarexModifications.newGameMinusMinusVersion)) ? "inline-block" : "none"
    } catch (err) {
        console.log(err)
        updateChallenges()
    }
...
function isIC3Trapped() {
	return !player.challenges.includes("postc3") && player.aarexModifications.newGameMinusMinusVersion !== undefined
}
...
function updateInfCosts() {
    ...
    if (document.getElementById("replicantis").style.display == "block" && document.getElementById("infinity").style.display == "block") {
        document.getElementById("replicantichance").innerHTML = "Replicate chance: "+Math.round(player.replicanti.chance*100)+"%" + (player.replicanti.chance < 1 ? "<br>+1% Cost: "+shortenCosts(player.replicanti.chanceCost)+" IP" : "")
        let replGalOver = 0
...
function replicantiGalaxy() {
	var maxGain=Math.floor(player.replicanti.gal*(player.timestudy.studies.includes(131)?1.5:1))-player.replicanti.galaxies
	if (player.replicanti.amount.lt(Number.MAX_VALUE)||maxGain<1) return
	var divide=player.achievements.includes("r126")
	var galaxyGain=divide&&player.dilation.upgrades.includes("ngpp4")&&player.eternities>=2e10&&player.aarexModifications.newGame3PlusVersion?Math.min(Math.floor(player.replicanti.amount.log(Number.MAX_VALUE)),maxGain):1
	player.replicanti.galaxies+=galaxyGain
	player.replicanti.amount=Decimal.div(divide?player.replicanti.amount:1,Decimal.pow(Number.MAX_VALUE,galaxyGain)).max(1)
	player.galaxies-=1
	galaxyReset()
}
...
function updateMilestones() {
    var moreUnlocked = player.aarexModifications.newGame3PlusVersion && player.dilation.upgrades.includes("ngpp2")
...
function galaxyReset() {
    ...
    if (!player.aarexModifications.newGameMinusMinusVersion && player.infinitied < 1 && player.eternities == 0) {
...
function changeSaveDesc(saveId, placement) {
	...
	try {
		...
		var message=temp.aarexModifications.newGameMinusVersion?"NG-, ":""
		if (temp.aarexModifications.newGameMinusMinusVersion) message+="NG--, "
		if (temp.aarexModifications.newGamePlusPlusVersion) message+="NG++"+(temp.aarexModifications.newGame3PlusVersion?"+":"")+", "+(temp.aarexModifications.newGamePlusVersion?"":"No NG+ features, ")
...
function toggle_mode(id) {
	...
	document.getElementById(id+"Btn").textContent=(id=="ngpp"?"NG++":id=="ngp"?"NG+":id=="ngmm"?"NG--":"NG-")+": "+(modes[id]>1?"NG+++":modes[id]?"ON":"OFF")
	if (id=="ngpp"&&modes.ngpp&&!modes.ngp) toggle_mode("ngp")
}
...
function showNextModeMessage() {
	if (ngModeMessages.length>0) {
		...
		document.getElementById("welcomeMessage").textContent = ngModeMessages[ngModeMessages.length-1]
		ngModeMessages.pop()
...
document.getElementById("bigcrunch").onclick = function () {
    ...
    if ((player.money.gte(Number.MAX_VALUE) && !player.currentChallenge.includes("post")) || (player.currentChallenge !== "" && player.money.gte(player.challengeTarget))) {
        ...
        if (player.money.gte("1e5000") && isIC3Trapped()) player.challenges.push("postc3")
...
function startDilatedEternity() {
    ...
    if (!onActive && player.aarexModifications.dilationConf) if (!confirm("Dilating time will start a new eternity, and all of your Dimension/Infinity Dimension/Time Dimension multiplier's exponents and tickspeed multiplier's exponent will be reduced to ^ 0.75. If you can eternity while dilated, you'll be rewarded with tachyon particles based on your antimatter and tachyon particles.")) return
    clearInterval(gameLoopIntervalId);
...
function getRebuyableDilUpgCost(id) {
	...
	var costScalingStart = Math.floor(Math.log10(1e63 / costGroup[0]) / Math.log10(costGroup[1]))
	return new Decimal(costGroup[0]).times(Decimal.pow(costGroup[1],amount + Math.max(amount - costScalingStart + 1, 0) * (amount - costScalingStart + 2) / 4))
}

function updateDilationUpgradeCosts() {
    var dil1cost = getRebuyableDilUpgCost(1)
    document.getElementById("dil1cost").textContent = "Cost: " + (dil1cost.gt(1e63) ? formatValue(player.options.notation, dil1cost, 1, 1) : shortenCosts(dil1cost)) + " dilated time"
...
function gainDilationGalaxies() {
	if (player.dilation.nextThreshold.lte(player.dilation.dilatedTime)) {
		...
		let oldThresholdExp = thresholdGalaxies + Math.max(thresholdGalaxies - 449, 0) * (thresholdGalaxies - 448) / 4
		let timesGained = player.dilation.dilatedTime.div(player.dilation.nextThreshold).log(thresholdMult) + 1 + oldThresholdExp
		if (timesGained > 450) timesGained = Math.sqrt(1.25 * 1.25 - 450 + timesGained) * 2 + 447.5
		player.dilation.freeGalaxies = Math.floor(timesGained) * galaxyMult
		player.dilation.nextThreshold = Decimal.pow(thresholdMult, timesGained + Math.max(timesGained - 449, 0) * (timesGained - 448) / 4 - oldThresholdExp).times(player.dilation.nextThreshold)
		giveAchievement("Universal harmony")
	}
}
...
setInterval(function() {
    ...
    if (player.infinitied == 0 && player.infinityPoints.lt(new Decimal(1e50)) && player.infinityUpgrades.length < 1 && player.eternities <= 0) document.getElementById("infinityPoints2").style.display = "none"
...
function gameLoop(diff) {
    ...
    if (player.eternities > 0) document.getElementById("tdtabbtn").style.display = ""
    ...
    document.getElementById("idtabbtn").style.display = (player.infDimensionsUnlocked[0] || player.eternities > 0) ? "" : "none"
    ...
    let gain;
    if (player.timeShards.gt(player.tickThreshold)) {
    ...
    if (player.eternities == 0 || isEmptiness) {
    ...
    if (player.money.gte(Number.MAX_VALUE) && (!player.break || (player.currentChallenge != "" && player.money.gte(player.challengeTarget)) || (isIC3Trapped() && player.money.gte("1e5000")))) {
    ...
    document.getElementById("postInfinityButton").innerHTML = "<b>"+(IPminpeak.gt("1e30000003") && player.options.theme != "Aarex's Modifications" ? "Gain " : "Big Crunch for ")+shortenDimensions(gainedInfinityPoints())+" Infinity points.</b>" + (IPminpeak.gt("1e100000") && player.options.theme != "Aarex's Modifications" ? "" : "<br>"+shortenDimensions(currentIPmin) + " IP/min"+"<br>Peaked at "+shortenDimensions(IPminpeak)+" IP/min")
        ...
        if (interval <= replicantiTicks && player.replicanti.unl) {
            if (player.replicanti.amount.lte(100)) {
                ...
                if (Math.round(player.replicanti.chance) !== 1) {
                    ...
                    if (!player.timestudy.studies.includes(192)) player.replicanti.amount = player.replicanti.amount.min(Number.MAX_VALUE)
                    counter = 0
                } else {
    ...
    extraReplGalaxies = Math.floor(player.timestudy.studies.includes(225) ? player.replicanti.amount.e / 1e3 : player.timestudy.studies.includes(226) ? player.replicanti.gal / 40 : 0)
    ...
    if (player.infMultBuyer) {
        ...
        if (dif > 0) {
            ...
            if (player.autobuyers[11].priority !== undefined && player.autobuyers[11].priority !== null && player.autoCrunchMode == "amount") player.autobuyers[11].priority = Decimal.times(player.autobuyers[11].priority, Decimal.pow(2, dif));
            if (player.autoCrunchMode == "amount") document.getElementById("priority12").value = formatValue("Scientific", player.autobuyers[11].priority, 2, 0);
        }
    }
    ...
    document.getElementById("eternitybtn").innerHTML = (player.dilation.active||gainedEternityPoints().lt(1e6)||player.currentEternityChall!==""||player.options.theme=="Aarex's Modifications" ? "<b>" + (player.currentEternityChall!=="" ? "Other challenges await..." : player.eternities>0 ? "" : "Other times await...") + " I need to become Eternal.</b><br>" : "") + (player.eternities > 0 && (player.currentEternityChall==""||player.options.theme=="Aarex's Modifications") ? "Gain <b>"+shortenDimensions(gainedEternityPoints())+"</b> Eternity points." + ((player.dilation.active||EPminpeak.gte("1e30003"))&&player.options.theme!="Aarex's Modifications" ? "" : "<br>" + shortenDimensions(currentEPmin)+" EP/min") + (player.dilation.active&&player.options.theme!="Aarex's Modifications" ? "" : "<br>Peaked at "+shortenDimensions(EPminpeak)+" EP/min<br>") + (player.dilation.active ? "<br>+" + shortenMoney(Math.max(getDilGain() - player.dilation.totalTachyonParticles, 0)) +" Tachyon particles." : "") : "")
    updateMoney();
    updateCoinPerSec();
    updateDimensions()
    updateInfCosts()
    ...
    if (Decimal.gt(player.galaxyPoints, 0) && !isEmptiness) {
        document.getElementById("galaxybtn").style.display = "inline-block"
        document.getElementById("galaxyPoints").innerHTML = "You have <span id='GP'>"+shortenDimensions(player.galaxyPoints)+"</span> Galaxy point"+(player.galaxyPoints.gt(1)?"s.":".")
    } else {
        document.getElementById("galaxybtn").style.display = "none";
    }
    if ((player.infinityPoints.gt(0) || player.eternities !== 0 || player.infinityUpgrades.length != 0) && !isEmptiness) {
    ...
    if (player.money.gt(Decimal.pow(10, 3 * 86400 * 365.2425 * 79.3 / 10))) document.getElementById("infoScale").innerHTML = "<br>If you start writing down 3 numbers a second when you were an American baby,<br> you would waste "+(player.money.log10() / 3 / 86400 / 365.2425 / 79.3 * 100).toFixed(3)+"% of your average life while writing down your antimatter amount."
    ...
    if (player.dilation.upgrades.includes(10)) {
        ...
        if ((document.getElementById("timestudies").style.display != "none" || document.getElementById("masterystudies").style.display != "none") && document.getElementById("eternitystore").style.display != "none") {
...
function autoBuyerTick() {
    ...
    if (player.eternities >= 100 && player.eternityBuyer.isOn) {
        if (player.autoEterMode === undefined || player.autoEterMode == "amount") {
            ...
            if (player.peakSpent >= new Decimal(player.eternityBuyer.limit).toNumber()) eternity(false, true)
        } else if (gainedEternityPoints().gte(player.lastTenEternities[0][1].times(player.eternityBuyer.limit))) eternity(false, true)
    }

...
function updateNewPlayer(reseted) {
    if (reseted) {
        var modesChosen = {
            ...
            ngpp: player.meta == undefined ? (player.blackhole == undefined ? false : 3) : tmp.ngp3 ? (player.exdilation !== undefined ? 4 : 2) : true,
...
function updateTemp() {
	...
	if (tmp.ngp3) {
		if (player.ghostify.ghostlyPhotons.unl) {
			...
			tmp.bru[4]=1 //BRU19
			tmp.bru[5]=1 //BRU20
			tmp.nu[4]=Decimal.pow(player.ghostify.ghostParticles.add(1).log10(),Math.pow(player.quantum.colorPowers.r.add(player.quantum.colorPowers.g).add(player.quantum.colorPowers.b).add(1).log10()*0,1)+1).max(1) //NU14
			tmp.nu[5]=1 //NU15
		}
		tmp.ns=nanospeed
		if (ghostified) {
...
function updateMoney() {
	document.getElementById("coinAmount").textContent = formatValue(player.options.notation, player.money, 2, 1)
...
function getEternitied() {
	...
	if (player.eternitiesBank) if (inQC(0)||(hasNU(10)?player.quantum.bigRip.active:false)) total = nA(total, player.eternitiesBank)
	return total
}
...
function getGalaxyCostScalingStart(galaxies, scalingSpeed) {
    ...
    if (tmp.ngp3) if (player.quantum.bigRip.active && player.quantum.bigRip.upgrades.includes(15)) n += Math.sqrt(player.eternityPoints.add(1).log10()) * 3.55
...
function getGalaxyRequirement(offset=0, display) {
	...
	let scaling = 0
	if (player.currentChallenge == "challenge4") amount += player.tickspeedBoosts == undefined ? 19 : -30
...
function getGalaxyCostIncrease() {
	...
	if (player.currentChallenge == "challenge4") ret = player.tickspeedBoosts == undefined ? 90 : 60
...
function getDilTimeGainPerSecond() {
	...
	if (tmp.ngp3 ? !player.quantum.bigRip.active || player.quantum.bigRip.upgrades.includes(11) : false) {
...
function updateDimensions() {
    if (document.getElementById("antimatterdimensions").style.display == "block" && document.getElementById("dimensions").style.display == "block") {
        var shown = getEternitied() > 29
        for (let tier = 8; tier > 0; tier--) {
			shown = shown || canBuyDimension(tier)
            var name = TIER_NAMES[tier];
            document.getElementById(tier+"Row").style.display = shown ? "" : "none";
            if (shown) {
    ...
    if (document.getElementById("stats").style.display == "block" && document.getElementById("statistics").style.display == "block") {
        ...
        document.getElementById("bestmoneythisrip").textContent = (!tmp.ngp3 ? false : player.quantum.bigRip.active) ? 'Your best antimatter for this big rip is ' + shortenMoney(player.quantum.bigRip.bestThisRun) + "." : ""
        document.getElementById("bestmoneybigrip").textContent = (!tmp.ngp3 ? false : ghostified || player.quantum.bigRip.times) ? 'Your best antimatter for all big rips is ' + shortenMoney(player.quantum.bigRip.bestAntimatter) + "." : ""
        document.getElementById("totalmoneybigrip").textContent = (!tmp.ngp3 ? false : ghostified || player.quantum.bigRip.times) ? 'You have made a total of ' + shortenMoney(player.quantum.bigRip.totalAntimatter) + ' antimatter in all big rips.' : ""
        ...
        if (player.meta ? player.quantum.times < 1 : true) document.getElementById("quantumStatistics").style.display = "none"
            ...
            document.getElementById("bigRipped").textContent = (!tmp.ngp3 ? false : player.quantum.bigRip.times) ? "You have big ripped the universe "+getFullExpansion(player.quantum.bigRip.times)+" times." : ""
    ...
    if (document.getElementById("infinity").style.display == "block") {
        if (document.getElementById("preinf").style.display == "block") {
            ...
            else if (player.infinityPoints.gte(player.galacticSacrifice?5e6:5e9)) document.getElementById("postinfi23").className = "infinistorebtn1"
            ...
            if (player.galacticSacrifice&&(player.infinityDimension3.amount.gt(0)||player.eternities>(player.aarexModifications.newGameMinusVersion?-20:0)||quantumed)) {
                ...
                document.getElementById("postinfi50").innerHTML = "Dimension boost cost increases by 0.5 less.<br>"+getDimboostCostIncrease()+(player.infinityUpgrades.includes("postinfi50")?"":" -> "+(getDimboostCostIncrease()-0.5))+"<br>Cost: "+shortenCosts(1e25)+" IP"
                document.getElementById("postinfi51").innerHTML = "Galaxies are 20% more stronger.<br>Cost: "+shortenCosts(1e29)+" IP"
                document.getElementById("postinfi52").innerHTML = "Galaxy cost increases by 3 less.<br>"+getGalaxyCostIncrease()+(player.infinityUpgrades.includes("postinfi50")?"":" -> "+(getGalaxyCostIncrease()-3))+"<br>Cost: "+shortenCosts(1e33)+" IP"
...
function updateEternityChallenges() {
	...
	if (quantumed&&tmp.ngp3) document.getElementById("autoEC").className=player.quantum.autoEC?"timestudybought":"storebtn"
}
...
function updateInfCosts() {
    ...
    if (document.getElementById("timestudies").style.display == "block" && document.getElementById("eternitystore").style.display == "block") {
        ...
        if (tmp.ngp3) {
            ...
            if (tmp.rg4) ng *= 0.4
            let exp = 0.2
            if (player.galaxies >= 1e4) exp *= Math.max(6 - player.galaxies / 2e3, 0)
            document.getElementById("232desc").textContent = "Currently: "+(Math.pow(1 + ng / 1000, exp) * 100 - 100).toFixed(1)+"%"
        }
    }
...
function getPostC3RewardMult() {
	...
	if (tmp.ngp3 ? player.quantum.bigRip.active : false) {
	...
	if (tmp.ngp3) realnormalgalaxies = Math.max(player.galaxies-player.quantum.electrons.sacGals,0)
...
function galaxyReset() {
    ...
    if (tmp.ngp3) if (player.quantum.autoOptions.sacrifice) sacrificeGalaxy(6, true)
    updateTickSpeed();
};
...
function getIPMult() {
	...
	if (player.galacticSacrifice&&player.tickspeedBoosts==undefined) {
		...
		if (player.achievements.includes("r55")) mult = mult.times(Math.max(Math.log10(6000/Math.max(player.bestInfinityTime, 1)), 1));
...
function setAchieveTooltip() {
    ...
    many.setAttribute('ach-tooltip', "Complete the Second Dimension Autobuyer challenge in 3 minutes or less. Reward: All dimensions are stronger in first 3 minutes of infinity"+(player.tickspeedBoosts==undefined?".":" and you keep Galaxy points on Infinity."));
    ...
    infchall.setAttribute('ach-tooltip', "Complete an infinity challenge."+(player.tickspeedBoosts===undefined&&player.galacticSacrifice?" Reward: Increase galaxy and g11 effectiveness based on IC's completed.":""))
...
function updatePriorities() {
    ...
    if (tmp.ngp3) {
        ...
        if (!isNaN(break_infinity_js ? quantumValue : quantumValue.l) && player.quantum.autobuyer) player.quantum.autobuyer.limit = quantumValue
        if (player.eternityBuyer.isOn&&player.eternityBuyer.dilationMode&&player.eternityBuyer.statBeforeDilation>=player.eternityBuyer.dilationPerAmount&&!player.eternityBuyer.slowStopped&&player.eternityBuyer.dilMode=="amount") {
            startDilatedEternity(true)
            return
        }
...
function toggleAutoBuyers() {
    ...
    if (tmp.ngp3) player.quantum.autobuyer.enabled = !bool
    updateCheckBoxes()
    updateAutobuyers()
}
...
function eternity(force, auto, presetLoad, dilated) {
    ...
    if ((player.infinityPoints.gte(Number.MAX_VALUE) && id7unlocked && (!player.options.eternityconfirm || auto || confirm("Eternity will reset everything except achievements and challenge records. You will also gain an Eternity point and unlock various upgrades."))) || force === true) {
        ...
        if (player.dilation.active) array = [player.thisEternity, getDilGain().sub(player.dilation.totalTachyonParticles).max(0), "d2"]
            ...
            player.quantum.breakEternity.eternalMatter = player.quantum.breakEternity.eternalMatter.add(getEMGain())
            if (player.ghostify.milestones < 15) player.quantum.breakEternity.eternalMatter = player.quantum.breakEternity.eternalMatter.round()
            array = [player.thisEternity, getEMGain(), "b"]
            updateBreakEternity()
            if (player.quantum.breakEternity.eternalMatter.gte(9.999999e99)) giveAchievement("This achievement doesn't exist 4")
        }
        ...
        if (player.currentEternityChall !== "") {
            ...
            if (tmp.ngp3?player.quantum.autoEC&&player.eternityChalls[player.currentEternityChall]<5:false) {
        ...
        } else if (tmp.ngp3) delete player.quantum.autoECN
        for (var i=0; i<player.challenges.length; i++) {
...
function unlockEChall(idx) {
    if (player.eternityChallUnlocked == 0) {
        ...
        if (tmp.ngp3) delete player.quantum.autoECN
    }
    updateEternityChallenges()
...
function buyDilationUpgrade(id, max) {
    if (id > 3 && id != 11) { // Not rebuyable
        ...
        if (id == 10 && tmp.ngp3) player.quantum.wasted = false
        if (id == 14) {
            updateMilestones()
...
function getPassiveTTGen() {
	...
	if (tmp.ngp3) if (player.quantum.bigRip.active) return normal
...
setInterval(function() {
    ...
    if (tmp.ngp3) if (player.quantum.bigRip.active) bigRipped = true
    ...
    if (tmp.ngp3) {
        ...
        if (!player.achievements.includes("ng3p65")) {
            for (var u=1;u<9;u++) {
                ...
                else if (player.quantum.nanofield.rewards>20&&u>7) giveAchievement("But I don't want to grind!")
            }
        }
        if (player.replicanti.amount.e >= 36e6) giveAchievement("Will it be enough?")
        if (player.quantum.bigRip.active) {
            let ableToGetRid7 = ableToGetRid2 && player.epmult.eq(1)
            let ableToGetRid8 = ableToGetRid7 && !player.quantum.breakEternity.did
        ...
        if (player.quantum.bigRip.spaceShards.e>32&&!player.quantum.breakEternity.did) giveAchievement("Finite Time")
    }
    ...
    if (tmp.ngp3) {
        ...
        if (player.money.gte(Decimal.pow(10,62e8))&&player.quantum.bigRip.active&&!player.ghostify.ghostlyPhotons.unl) {
...
function updateEPminpeak(diff, type) {
	if (type == "EP") {
		...
		var oldPoints = player.quantum.breakEternity.eternalMatter
	}
...
function gameLoop(diff) {
    ...
    if (player.matter.pow(20).gt(player.money) && (player.currentChallenge == "postc7" || (inQC(6) && !player.achievements.includes("ng3p34")))) {
        if (tmp.ngp3 ? player.quantum.bigRip.active && tmp.ri : false) {}
        else if (inQC(6)) {
            document.getElementById("challfail").style.display = "block"
    ...
    if (!tmp.ri) {
        ...
        if (tmp.ngp3 && player.quantum.bigRip.active) player.quantum.bigRip.totalAntimatter = player.quantum.bigRip.totalAntimatter.add(tempa)
    ...
    if (ghostified && isAutoGhostsSafe) {
        ...
        if (isAutoGhostActive(13)) {
            if (player.quantum.bigRip.active) {
                if (player.quantum.time>=player.ghostify.automatorGhosts[13].u*10) quantumReset(true,true,0,false)
            } else if (player.quantum.time>=player.ghostify.automatorGhosts[13].t*10) bigRip(true)
        }
        if (isAutoGhostActive(15)) if (player.quantum.bigRip.active&&getGHPGain().gte(player.ghostify.automatorGhosts[15].a)) ghostify(true)
    }
    if (tmp.ngp3) {
        var colorShorthands=["r","g","b"]
        ...
        colorBoosts.b=Decimal.pow(10,Math.sqrt(player.quantum.colorPowers.b.add(1).log10())*(player.ghostify.ghostlyPhotons.unl?tmp.le[4]:1))
        ...
        if (player.ghostify.ghostlyPhotons.unl) {
            ...
            data[player.quantum.bigRip.active?"amount":"darkMatter"]=data[player.quantum.bigRip.active?"amount":"darkMatter"].add(getGPHProduction().times(diff/10))
        ...
        gatheredQuarksBoost = Math.pow(player.quantum.replicants.quarks.add(1).log10(),player.masterystudies.includes("t362")?0.35:0.25)*0.67*(player.masterystudies.includes("t412")?1.25:1)*(player.ghostify.ghostlyPhotons.unl?tmp.le[3]:1)
    ...
    if (tmp.ngp3) if (player.masterystudies.includes("t332")) interval = Decimal.div(interval, player.galaxies)
    ...
    if (tmp.ngp3) if (player.masterystudies.includes("d10") && player.quantum.autoOptions.replicantiReset && player.replicanti.amount.gt(player.quantum.replicants.requirement)) replicantReset()
    ...
    if (player.infMultBuyer && (!player.boughtDims || canBuyIPMult())) {
        ...
        if (dif > 0) {
            ...
            player.infinityPoints = player.infinityPoints.minus(player.infMultCost.dividedBy(player.aarexModifications.newGameExpVersion?4:10))
    ...
    document.getElementById("quantumbtnFlavor").textContent = ((player.quantum!==undefined?!player.quantum.times&&(player.ghostify!==undefined?!player.ghostify.milestones:true):false)||!inQC(0)?((tmp.ngp3 ? player.quantum.bigRip.active : false)?"I am":inQC(0)?"My computer is":player.quantum.challenge.length>1?"Paired challenges are":"My challenging skills are")+" not powerful enough... ":"") + "I need to go quantum."
    var showGain = quantumed && (inQC(0)||player.options.theme=="Aarex's Modifications") ? "QK" : ""
    if (tmp.ngp3) if (player.quantum.bigRip.active) showGain = "SS"
    ...
    var showGHPPeakValue = GHPminpeakValue.lt(1e30)||player.options.theme=="Aarex's Modifications"
    ...
    if (tmp.ngp3) {
        ...
        if (player.ghostify.milestones>7||player.achievements.includes("ng3p66")) {
            if (player.quantum.bigRip.active) {
                player.quantum.bigRip.spaceShards=player.quantum.bigRip.spaceShards.add(getSpaceShardsGain().times(diff/1e3))
                if (tmp.be&&player.ghostify.milestones>14) player.quantum.breakEternity.eternalMatter=player.quantum.breakEternity.eternalMatter.add(getEMGain().times(diff/1e3))
            } else {
                var percentage=0.01
                if (player.ghostify.milestones>7&&player.achievements.includes("ng3p66")) percentage=0.02
                player.quantum.quarks=player.quantum.quarks.add(quarkGain().sqrt().times(diff/10))
                player.quantum.gluons.rg=player.quantum.gluons.rg.add(player.quantum.usedQuarks.r.min(player.quantum.usedQuarks.g).sqrt().times(diff/10))
                player.quantum.gluons.gb=player.quantum.gluons.gb.add(player.quantum.usedQuarks.g.min(player.quantum.usedQuarks.b).sqrt().times(diff/10))
                player.quantum.gluons.br=player.quantum.gluons.br.add(player.quantum.usedQuarks.b.min(player.quantum.usedQuarks.r).sqrt().times(diff/10))
                if (player.achievements.includes("ng3p66")) player.quantum.quarks=player.quantum.quarks.add(quarkGain().times(diff/1e3))
            }
            updateQuarkDisplay()
    ...
    setAndMaybeShow("quantumClock", tmp.ngp3 ? (quantumed && player.quantum.times > 1 && speedrunMilestonesReached < 28) : false, '"Quantum time: <b class=\'QKAmount\'>"+timeDisplayShort(player.quantum.time)+"</b>"')
...
function autoBuyerTick() {
    ...
    if (tmp.ngp3) if (speedrunMilestonesReached>22&&player.quantum.autobuyer.enabled&&!player.quantum.bigRip.active) {

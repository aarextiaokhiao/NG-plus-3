...
function getBreakEternityTDMult(tier){
	...
	if (brSave.upgrades.includes(13)) ret = ret.times(player.replicanti.amount.max(1).pow(1e-6))
...
function doNGMatLeast4TDChanges(tier, ret){
	...
	if (player.galacticSacrifice.upgrades.includes(44) && aarMod.ngmX >= 4) {
...
function getTimeDimensionPower(tier) {
	...
	if (aarMod.ngmX >= 4) ret = doNGMatLeast4TDChanges(tier,ret)
...
function getTimeDimensionProduction(tier) {
  	...
  	if (aarMod.ngmX>3&&(inNC(2)||player.currentChallenge=="postc1"||player.pSac!=undefined)) ret = ret.times(player.chall2Pow)
  	if (player.currentEternityChall == "eterc7") ret = dilates(ret.dividedBy(player.tickspeed.dividedBy(1000)))
  	if (aarMod.ngmX>3&&(tier>1||!player.achievements.includes("r12"))) ret = ret.div(100)
...
function getTimeDimensionDescription(tier) {
	if (!isTDUnlocked(((inNC(7) && aarMod.ngmX > 3) || inQC(4) || player.pSac!=undefined ? 2 : 1) + tier)) return getFullExpansion(player['timeDimension' + tier].bought)
	...
	if (document.getElementById("timedimensions").style.display == "block" && document.getElementById("dimensions").style.display == "block") {
		...
		for (let tier = 1; tier <= 8; ++tier) {
			if (isTDUnlocked(tier)) {
				...
				document.getElementById("timeMax" + tier).textContent = (quantumed ? '':"Cost: ") + shortenDimensions(player["timeDimension" + tier].cost) + (aarMod.ngmX > 3 ? "" : " EP")
...
function buyTimeDimension(tier) {
	var dim = player["timeDimension"+tier]
	if (aarMod.ngmX > 3 && getAmount(1) < 1) {
...
function getOrSubResourceTD(tier, sub) {
	if (sub == undefined) {
		...
		var maxval = player.achievements.includes("r36") ? currmax : E(Number.MAX_VALUE)
		if (aarMod.ngmX > 3) return player.money.min(maxval)
		return player.eternityPoints
	} else {
		if (aarMod.ngmX > 3) player.money = player.money.sub(player.money.min(sub))
...
function buyMaxTimeDimension(tier, bulk) {
	...
	if (aarMod.ngmX > 3 && getAmount(1) < 1) return
